...
int ModbusSlave::receive_request(unsigned char *received_string) 
{
        ...
        while (mySerialPort.available()) {
                received_string[bytes_received] = mySerialPort.read();
                bytes_received++;
...
ModbusSlave::ModbusSlave(HardwareSerial& s)
  : mySerialPort(s)
{
  configure(DEFAULT_TX_PIN, DEFAULT_SLAVE_ADDR, 9600L, SERIAL_8N1);
}

ModbusSlave::ModbusSlave(HardwareSerial& s, char txenpin, unsigned char slave)
  : mySerialPort(s)
{
  configure(txenpin, slave, 9600L, SERIAL_8N1);  
}
 ...
 * baud: baudrate in bps (typical values 9600 (default), 19200... 115200)
 * parity: http://arduino.cc/en/Serial/begin, default SERIAL_8N1 (no parity)
...
void ModbusSlave::configure(char txenpin, unsigned char slave, long baud, uint8_t parity)
{
	this->slave = slave;
	this->txenpin = txenpin;
	mySerialPort.begin(baud, parity);
...
unsigned int regs_size) 
{
        ...
        unsigned int start_addr;
        int exception;
        int length = mySerialPort.available();
